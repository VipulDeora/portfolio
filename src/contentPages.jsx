...
export function useRoute() {
  const [routeKey, setRouteKey] = React.useState(getRouteKey);
  ...
  const [path, hash = ""] = routeKey.split("#");
  return { key: routeKey, path: path || "/", hash: hash ? `#${hash}` : "" };
}
  ...
  return (
    <div id="related" className="scroll-mt-28">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {paths.map((path) => (
          <InternalLink key={path} to={path} className="group rounded-[1.2rem] border border-[#B9AD9B]/38 bg-[#F1EBDD]/56 p-5 shadow-[0_14px_44px_rgba(26,26,24,0.028)] transition hover:bg-[#EFE9DD]/88">
          ...
          <Surface className="p-5">
            ...
            <div className="grid gap-2">
              {documents.map((document) => (
                <InternalLink key={document.path} to={document.path} className={`rounded-[0.9rem] px-3 py-3 text-xs leading-5 transition ${document.path === activePath ? "bg-[#2F4D72] text-[#F7F4ED]" : "text-[#4F4A43] hover:bg-[#FBF8F0]/72 hover:text-[#171513]"}`}>
          ...
          <Surface className="p-5">
            ...
            <div className="grid gap-2">
              {connected.slice(0, 4).map((path) => (
                <InternalLink key={path} to={path} className="rounded-[0.9rem] px-3 py-3 text-xs leading-5 text-[#4F4A43] transition hover:bg-[#FBF8F0]/72 hover:text-[#2F4D72]">
                  {getLinkTitle(path)}
                </InternalLink>
              ))}
            </div>
  ...
  return (
    <article className="relative z-10">
      <section className="mx-auto max-w-7xl px-5 pb-14 pt-16 md:px-10 md:pb-18 md:pt-24">
        <InternalLink to={`/${document.family}`} className="mb-12 inline-flex items-center gap-3 text-[10px] uppercase tracking-[0.24em] text-[#4F4A43] transition hover:text-[#2F4D72]">
...
export function ContentRouter({ route }) {
